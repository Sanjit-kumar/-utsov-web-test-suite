...
test.describe('Database & API Security', () => {

  test('api/ directory does not list files (no directory listing)', async ({ request }) => {
    const res = await request.get('https://www.utsov.org/api/');
    // Should be 403 Forbidden or redirect, not 200 with file listing
    expect(res.status()).not.toBe(200);
  });

  test('SQLite database file is not publicly accessible', async ({ request }) => {
    const res = await request.get('https://www.utsov.org/api/db/capture.db');
    expect([403, 404]).toContain(res.status());
  });

  test('No .env file accessible from web root', async ({ request }) => {
    const res = await request.get('https://www.utsov.org/.env');
    expect([403, 404]).toContain(res.status());
  });

  test('No .git directory accessible from web', async ({ request }) => {
    const res = await request.get('https://www.utsov.org/.git/config');
    expect([403, 404]).toContain(res.status());
  });

  test('No firebase.json config file accessible', async ({ request }) => {
    const res = await request.get('https://www.utsov.org/firebase.json');
    expect([403, 404]).toContain(res.status());
  });

  test('Contact form API rejects malformed JSON gracefully', async ({ request }) => {
    const res = await request.post('https://www.utsov.org/api/contacts.php', {
      data: 'not-valid-json',
      headers: { 'Content-Type': 'application/json' },
    });
    // Should return 200 with an error object, not a 500
